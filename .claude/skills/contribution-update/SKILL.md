...
표 형식: `| 기여일자 | Pull request | PR 설명 | 기여 성격 | 기여 결과 | 반영 방식 |`

- **기여일자**: PR 생성일(`createdAt`). 표는 날짜 내림차순, 같은 날짜면 생성 시각이 늦은 것이 위.
- **Pull request**: `[repo#번호](URL)` 형식. `spring-projects/` 소유자 접두사는 생략하고 저장소 이름만 쓴다(다른 소유자와 헷갈릴 때만 `owner/repo#번호`).
- **PR 설명**: 한국어 한 줄. 클래스·프로퍼티 이름 등 식별자는 원문 그대로 둔다.
